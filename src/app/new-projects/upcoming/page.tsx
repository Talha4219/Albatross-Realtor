...
import { CalendarClock, AlertTriangle, Building } from 'lucide-react';
...
export default function UpcomingProjectsPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      ...
      setError(null);
      try {
        const res = await fetch('/api/projects?status=Upcoming');
        if (!res.ok) {
        ...
        if (data.success) {
          ...
          throw new Error(data.error || 'Could not load upcoming projects.');
        }
      } catch (err) {
    ...
    <div className="space-y-8">
      <Card className="shadow-lg border-primary/20 bg-gradient-to-br from-primary/5 via-background to-background">
        <CardHeader className="text-center">
            <CalendarClock className="mx-auto h-12 w-12 text-primary mb-3" />
            <CardTitle className="text-4xl font-headline text-primary">Upcoming New Projects</CardTitle>
            <CardDescription className="text-lg text-muted-foreground max-w-xl mx-auto">
                Get a sneak peek at new real estate projects launching soon. Be the first to know about pre-launch offers.
            </CardDescription>
        </CardHeader>
      </Card>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
         ...
         <div className="text-center py-16 border border-dashed rounded-lg bg-card">
          ...
          <h2 className="text-2xl font-semibold text-muted-foreground mb-2">No Upcoming Projects Found</h2>
          <p className="text-muted-foreground mb-6">
            There are no projects marked as 'Upcoming' at the moment. Check back soon!
          </p>
          <Button asChild>
